...
app.get("/", function(req, res) {
  ...
  Post.find(function(err, foundPosts) {
    if (foundPosts === 0) {
      ...
      res.render("home", {homeStartingContent: homeStartingContent, _: _, posts: foundPosts});
    }
  });
});
...
app.post("/compose", function(req, res) {
  const userPostTitle = req.body.postTitle;
  const userPostBody = req.body.postContent;

  const post = new Post({
    title: userPostTitle,
    content: userPostBody
  });
